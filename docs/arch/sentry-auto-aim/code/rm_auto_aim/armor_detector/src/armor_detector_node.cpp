...
: Node("armor_detector", options) {
  ...
  // Initialize last save time to 0 so first frame triggers a save
  last_save_time_ = rclcpp::Time(0, 0, this->get_clock()->get_clock_type());
}
  ...
  if (ba_solver_ != nullptr && pnp_solver_ != nullptr) {
    ...
    for (auto &armor : armors) {
      ...
      if (pnp_solver_->solvePnP(
        ...
        // Fill the distance to image center - now use the real physical distance
        // Calculate the 3D Euclidean distance from origin (0,0,0) to the armor position
        double real_distance = std::sqrt(
            std::pow(tvec.at<double>(0), 2) + 
            std::pow(tvec.at<double>(1), 2) + 
            std::pow(tvec.at<double>(2), 2)
        );
        armor_msg.distance_to_image_center = real_distance;
...
std::vector<Armor> ArmorDetectorNode::detectArmors(
  ...
  // Save raw image every 60s
  auto current_time = this->now();
  if ((current_time - last_save_time_).seconds() >= 60.0) {
    last_save_time_ = current_time;
    std::string save_dir = "/home/cod-sentry/fyt2024-log/raw_images";
    std::filesystem::create_directories(save_dir);
    cv::Mat bgr_img;
    cv::cvtColor(img, bgr_img, cv::COLOR_RGB2BGR);
    std::string filename = save_dir + "/" + std::to_string(current_time.seconds()) + ".jpg";
    cv::imwrite(filename, bgr_img);
    FYT_INFO("armor_detector", "Saved raw image: {}", filename);
  }

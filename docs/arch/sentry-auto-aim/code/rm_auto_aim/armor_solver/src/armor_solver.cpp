...
rm_interfaces::msg::GimbalCmd Solver::solve(const rm_interfaces::msg::Target &target,
  ...
  switch (state) {
    case TRACKING_ARMOR: {
      if (std::abs(target.v_yaw) > max_tracking_v_yaw_ && std::abs(distance)<1.5) {
        overflow_count_++;
      } else {
        overflow_count_ = 0;
      }
  ...
  if(std::abs(target.v_yaw) > 6.0){
      double raw_log  = std::log(std::abs(target.v_yaw)) * 0.4;

      double max_log_val = std::log(10.0);
      double offset = (raw_log / max_log_val) * 1.2;
      offset = std::clamp(offset,0.0,1.2);
      pitch -= offset*M_PI / 180.0;
  }
...
bool Solver::isOnTarget(const double cur_yaw,
  ...
  //double projected_half_w = armor_half_w;
